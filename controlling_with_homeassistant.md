...
- when we are producing solar power and if the battery is full, we just take what the house currently needs or what is produced
- when we are not producing solar power and the battery is discharging, we either limit the discharge or we take what the house needs (if it is below the discharge limit)
- when we are producing solar power and the battery should be charging, we either guarantee a minimum charge capacity or assign everything to charge the battery (if the produced power is below the charge guarantee)
- when we are not producing we either take what the house needs or the maximum discharge limit
...
action:
  - variables:
      current_demand: "{{ (states('sensor.e220_power_power_curr')|int + states('sensor.hm_600_p_ac')|int(0)) | int }}"
  ...
  - if:
      - condition: numeric_state
        ...
        above: 95
    then: # battery is full
      - choose:
          - conditions: # currently producing
              - condition: numeric_state
            ...
            sequence:
              - if:
                  - condition: template
                    value_template: "{{ states('sensor.solarinputpower')|int > current_demand }}"
                then: # producing more than demand at full battery => take only what we need
                  - service: input_number.set_value
                    data:
                      value: "{{ current_demand }}"
                    target:
                      entity_id: input_number.inverter_power_limit
                else: # producing less than demand at full battery => just take what we produce
                  - service: input_number.set_value
                    data:
                      value: "{{ states('sensor.solarinputpower')|int }}"
                    target:
                      entity_id: input_number.inverter_power_limit
          - conditions: #currently not producing
              - condition: numeric_state
                entity_id: sensor.solarinputpower
                below: 1.0
            sequence:
              - if:
                ...
                then:
                  ...
                  - service: input_number.set_value
                    ...
                    target:
                      entity_id: input_number.inverter_power_limit
    else: # battery should be charging
      - choose:
          - conditions: # currently producing
              - condition: numeric_state
                entity_id: sensor.solarinputpower
            ...
            sequence:
              - if:
                  - condition: template
                    value_template: "{{ states('sensor.solarinputpower')|int > minimum_charge }}"
                then: # producing more than demand, battery should charge => grant a minimum to charge
                  - service: input_number.set_value
                    data:
                      value: "{{ (states('sensor.solarinputpower')|int - minimum_charge|int) | int }}"
                    target:
                      entity_id: input_number.inverter_power_limit
                else: # producing less than demand at full battery => just take what we produce
                  - service: input_number.set_value
